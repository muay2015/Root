...
export default function App() {
  ...
  if (screen === 'landing') {
    ...
    content = sessionUserId && !isAnonymous ? (
      <AccountScreen
        email={sessionUserEmail}
        initialDisplayName={sessionDisplayName}
        syncMessage={syncMessage}
        onDisplayNameChange={setSessionDisplayName}
        onSignOut={() => {
          setSessionUserId(null);
          setIsAnonymous(true);
          setSessionUserEmail(null);
        ...
        <div className="flex items-center gap-2">
          {isAnonymous ? (
            ...
            <>
              <button
                onClick={() => onNavigate('account')}
                className="hidden h-11 items-center gap-2 border border-slate-300 bg-white px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50 sm:flex"
              >
                <Settings className="h-4 w-4" />
                {sessionDisplayName}님
              </button>
              <button
                onClick={onSignOut}
...
function BottomNavigation({ current, onNavigate }: { current: Screen; onNavigate: (screen: Screen) => void }) {
  ...
  const items: Array<{ id: Screen; label: string; icon: ReactNode }> = [
    ...
    { id: 'account', label: '계정', icon: <Settings className="h-5 w-5" /> },
  ];
