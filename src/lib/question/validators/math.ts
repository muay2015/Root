...
export function validateMathQuality(
  ...
  const isPositionRelationChoices =
    choices.length >= 3 &&
  ...
  const stimulusForCheck = String(question.stimulus ?? '');
  const combinedForMiddle = `${topic} ${stem} ${stimulusForCheck}`;
  // 6-a. 중등 수학 「원과 직선」 전용 체크
  if (isMiddleMath) {
    const isCircleLineQuestion =
      /원과\s*직선|직선과\s*원|원.*직선.*위치|직선.*원.*위치|중심.*에서.*직선.*거리|반지름.*직선/.test(
        combinedForMiddle,
      );
