...
export function stripAnswerLabel(value: string) {
  return value
...
export function parseAnswerChoiceIndex(value: string): number | null {
  ...
  const labeledDigit = compact.match(
    /^(?:[^0-9]*:)?(?:[^0-9]*)([1-5])(?:\uBC88|\uBC88\uC774\uB2E4|\uBC88\uC784|\uBC88\uC815\uB2F5|\uBC88\uC774\uC815\uB2F5)?$/u,
  );
  if (labeledDigit) {
    return Number.parseInt(labeledDigit[1], 10) - 1;
  }

  const embeddedDigit = stripped.match(
    /(?:^|[\s:([{<"'`-])([1-5])(?:[.)]|\uBC88)(?:$|[\s)\]}>.,:;!?"'`])/u,
  );
  if (embeddedDigit) {
...
export function resolveAnswerFromChoices(rawAnswer: unknown, choices: string[]) {
  if (rawAnswer && typeof rawAnswer === 'object') {
    const objectAnswer =
      (rawAnswer as Record<string, unknown>).text ??
      (rawAnswer as Record<string, unknown>).value ??
      (rawAnswer as Record<string, unknown>).content ??
      (rawAnswer as Record<string, unknown>).label;
...
export function countAnswerMatches(rawAnswer: unknown, choices: string[]) {
