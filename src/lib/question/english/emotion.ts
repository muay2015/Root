...
const KOREAN_CHOICE_DICTIONARY: Array<[RegExp, string]> = [
  [/\uae34\uc7a5(?:\ud558\ub2e4|\ud55c)?/gu, 'tense'],
  [/\ubd88\uc548(?:\ud558\ub2e4|\ud55c)?/gu, 'anxious'],
  [/\ucd08\uc870(?:\ud558\ub2e4|\ud55c)?/gu, 'nervous'],
...
const EMOTION_PAIR_FALLBACKS: Array<[RegExp, string]> = [
...
function splitStemParts(stem: string) {
...
function inferEmotionKind(stem: string, instruction: string, passage: string) {
  const combined = `${stem}\n${instruction}\n${passage}`;
  ...
  if (
    combined.includes(EMOTION_WORD) ||
...
function buildCanonicalStem(kind: 'emotion' | 'atmosphere', passage: string, fallbackStem: string) {
  if (kind === 'emotion') {
...
function translateKoreanChoiceToEnglish(choice: string) {
...
function normalizeEmotionPairChoice(choice: string) {
...
function normalizeAtmosphereChoice(choice: string) {
  const translated = translateKoreanChoiceToEnglish(choice);
  return translated.split(/\s*\/\s*/)[0].trim();
}

export function normalizeEmotionChangeChoices(choices: string[]) {
...
export function normalizeEmotionChangeAnswer(answer: unknown) {
