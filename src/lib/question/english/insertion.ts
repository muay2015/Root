 ...
 */
function extractStimulusBlock(text: string): { stimulus: string; cleaned: string } | null {
  const pattern =
    /(?:<보기>|\[보기\]|<주어진\s*문장>|\[주어진\s*문장\])\s*([\s\S]*?)(?=\n\n|\n\s*(?:①|②|③|④|⑤|\(\s*[1-5]\s*\))|$)/;
...
export interface SentenceInsertionStandardized {
  instruction: string;
  stimulus: string;
  passage: string;
}
