...
export async function updateAvatar(userId: string, file: File): Promise<Result<string>> {
  if (!supabase) {
    return { data: null, error: 'Supabase is not configured.' };
  }

  try {
    // 1. 이미지 업로드 (avatars 버킷)
    ...
    if (updateError) {
      return { data: null, error: normalizeSupabaseErrorMessage(updateError) };
    }
