...
export function validateMathQuality(question, index, input, reasons, _warnings, issueCounts) {
    ...
    const isCircleAngleQuestion = /원\s*위의\s*점|원\s*위에|원주각|내접각|외접각|할선|현\s*[A-Z]{2}|원에\s*내접|원에\s*외접/.test(combinedText);
    const isTriangleQuestion = /삼각형\s*[A-Z]{3}|△[A-Z]{3}|직각삼각형|이등변삼각형/.test(combinedText);
    const isAngleWithMultiplePoints = /∠[A-Z]{2,3}/.test(combinedText) &&
    ...
    if (isGeometryQuestion && !question.diagram_svg) {
        pushReason(reasons, issueCounts, 'math_missing_diagram', `Question ${index}: This is a geometry question (circle/triangle/angle) but diagram_svg is null. ` +
