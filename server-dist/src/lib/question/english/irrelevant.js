...
function normalizeIrrelevantSentenceSource(value) {
 ...
 * 숫자형 선택지 라벨인지 확인
 */
function isNumericChoiceLine(line) {
    return /^(?:\(?[1-5]\)?|[①-⑤])(?:\s*[.)-]?\s*)?$/.test(line.trim());
}
/**
 * 지문에서 숫자형 선택지 행 제거
 */
function stripNumericChoiceLines(text) {
    return text
        .split('\n')
        .map((line) => line.trim())
 ...
 * 발문 추출
 */
function extractIrrelevantInstruction(text) {
    const patterns = [
    ...
    return (lines.find((line) => /\?$/.test(line) &&
        (/overall flow/i.test(line) ||
            ...
            line.includes('관계없는 문장'))) ?? null);
}
function isIrrelevantInstructionLike(text) {
    const normalized = text.trim();
    if (!normalized)
        return false;
    return (/overall flow/i.test(normalized) ||
        normalized.includes('관계 없는 문장') ||
        normalized.includes('관계없는 문장') ||
        normalized.includes('고르시오') ||
        normalized.includes('가장 적절하지 않은 것은'));
}
function removeInstructionFromText(text, instruction) {
 ...
 * 선택지(문장) 텍스트 정규화
 */
function normalizeIrrelevantChoiceSentence(choice) {
 ...
 * 지문에 (1)~(5) 번호 주입
 */
function injectIrrelevantSentenceMarkers(passage, choiceSentences) {
...
function stripExistingIrrelevantMarkers(text) {
    return text.replace(/\(\s*[1-5]\s*\)\s*/g, '').trim();
}
function stripLeadingSentenceEnumeration(text) {
    return text.replace(/^\s*\d+\s*[.)]\s*/g, '').replace(/^\s*[①-⑤]\s*/g, '').trim();
}
function stripTrailingIrrelevantPrompt(text) {
    return text
        .replace(/\s*(?:가장\s*적절하지\s*않은\s*것은\??|다음\s*글에서\s*전체\s*흐름과\s*관계\s*없는\s*문장(?:을)?\s*(?:고르시오)?\.?\??)\s*$/giu, '')
        .trim();
}
function isValidIrrelevantSentenceCandidate(text) {
 ...
 * 지문을 개별 문장 후보로 분할
 */
function splitIrrelevantSentenceCandidates(text) {
...
export function standardizeEnglishIrrelevantSentence(params) {
    ...
    const instruction = extractIrrelevantInstruction(primarySource) ??
        extractIrrelevantInstruction(combinedSource) ??
        CANONICAL_IRRELEVANT_SENTENCE_INSTRUCTION;
    const extractPassage = (txt, inst) => {
