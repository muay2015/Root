...
function hasSummaryBlanks(value) {
    return /\(\s*A\s*\)/i.test(value) && /\(\s*B\s*\)/i.test(value);
}
function looksLikeEnglishSummarySentence(value) {
    return hasSummaryBlanks(value) && /[A-Za-z]{5,}/.test(value);
}
function stripLeadingSummaryLabel(value) {
    return value.replace(/^(?:Summary|Result|Conclusion)\s*[:\-]?\s*/i, '').trim();
}
function stripBlanksFromSummaryInstruction(value) {
    return value
        .split('\n')
        .map((line) => {
        const trimmed = line.trim();
        const isInstructionLine = /가장 적절한 것은\?|빈칸/i.test(trimmed) &&
...
export function normalizeSummaryCompletionChoices(choices) {
...
export function standardizeEnglishSummaryCompletion(params) {
    let stem = normalizeSummaryBlankMarkers(normalizeEnglishPlainText(params.stem.replace(/\r\n/g, '\n')));
    let stimulus = typeof params.stimulus === 'string'
        ? normalizeSummaryBlankMarkers(normalizeEnglishPlainText(params.stimulus))
        : null;
    const summarySentenceRegex = /((?:Summary|Result|Conclusion)?[:\s-]*[A-Z0-9][^.?!\n]*(?:\(\s*A\s*\)|\[\s*A\s*\])[^.?!\n]*(?:\(\s*B\s*\)|\[\s*B\s*\])[^.?!\n]*[.?!]?|(?:Summary|Result|Conclusion)?[:\s-]*[A-Z0-9][^.?!\n]*(?:\(\s*B\s*\)|\[\s*B\s*\])[^.?!\n]*(?:\(\s*A\s*\)|\[\s*A\s*\])[^.?!\n]*[.?!]?)/gi;
    const candidates = [
        ...stem.split(/\n\s*\n/),
        ...stem.split('\n'),
    ...
    stem = stem.replace(/\n{3,}/g, '\n\n').trim();
    // 복구: stem에 영어 지문이 없고 stimulus가 지문+요약문을 모두 포함하는 경우
    // (모델이 stem을 비우고 stimulus에 모든 내용을 넣은 케이스)
    const stemHasEnglishPassage = (text) => {
        const withoutInstruction = text.replace(/다음 글의 내용을 한 문장으로 요약할 때[^?]*\?/g, '').trim();
        return /[A-Za-z]{4,}/.test(withoutInstruction) && withoutInstruction.length > 80;
    };
    if (!stemHasEnglishPassage(stem) && stimulus && stimulus.length > 60) {
