 ...
 */
export function injectDataBlockMarkers(text) {
    if (/(<보기>|\[보기\]|<조건>|\[조건\]|<자료>|\[자료\])/.test(text))
        return text;
    const CIRCLE_JAMO_REGEX = /[㉠-㉭]/;
    if (CIRCLE_JAMO_REGEX.test(text)) {
        const isQuestionPart = (s) => /옳은 것은|바르게 설명|고른 것은|적절한 것은|맞는 것은|알맞은 것은|추론한 것은|설명한 것은/.test(s) ||
            s.trim().endsWith('?');
        ...
        if (firstJamoIndex > 0) {
            ...
            const sr = /[^.!?。]+[.!?。]+/g;
            let sm;
            while ((sm = sr.exec(raw)) !== null)
                sentParts.push(sm[0].trim());
            const tail = raw.substring(raw.search(/[^.!?。]+$/)).trim();
            if (tail && !sentParts.some(s => s.includes(tail)))
                sentParts.push(tail);
            ...
            if (beforeBlock.length > 0)
                resultParts.push(beforeBlock);
            resultParts.push('<보기>');
            const formattedBlock = cleanDataBlock
                .replace(/([^\n])\s*([㉠-㉭])/g, '$1\n$2');
            resultParts.push(formattedBlock);
            if (trailingQuestion.length > 0)
                resultParts.push(trailingQuestion);
    ...
    if (lastIndex !== -1) {
        ...
        if (tail.length > 0 && !sentences.some(s => s.includes(tail)))
            sentences.push(tail);
    }
    if (sentences.length < 3)
        return text;
    const isDataSentence = (s) => /^[A-Z]\s*(지역|항목|학생|씨|국|나라|언어|문화|팀|군)?\s*(은|는|이|가|의)\s+/.test(s) ||
        /^\(가\)|\(나\)/.test(s) ||
        /^[A-Z]형|^[A-Z]안/.test(s);
    const isQuestionSentence = (s) => /옳은 것은|바르게 설명|고른 것은|적절한 것은|맞는 것은|알맞은 것은|추론한 것은|설명한 것은/.test(s) ||
        s.trim().endsWith('?');
    let dataStart = -1;
    let dataEnd = -1;
    ...
    if (introParts.length > 0)
        result.push(introParts.join(' '));
    result.push('<자료>');
    result.push(dataParts.join('\n'));
    if (questionParts.length > 0)
        result.push(questionParts.join(' '));
    return result.join('\n');
}
/**
 * 지문에서 대화형 화자 정보를 추출하여 맵으로 만듭니다.
 */
export function buildSpeakerMapFromStem(stem) {
    const map = new Map();
    if (!stem)
        return map;
 ...
 */
export function findSpeakerForChoice(choiceText, speakerMap) {
    if (!choiceText || !speakerMap)
        return undefined;
    const clean = choiceText.replace(/^['"“”’‘]|['"“”’‘]$/g, '').trim();
    if (speakerMap.has(choiceText))
        return speakerMap.get(choiceText);
    if (speakerMap.has(clean))
        return speakerMap.get(clean);
